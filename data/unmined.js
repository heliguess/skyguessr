...
class Unmined {
    ...
    constructor(mapElement, options, regions) {
        ...
        const initialZoom = this.#options.zoom ?? this.#options.defaultZoom ?? (mapZoomLevels - this.#options.maxZoom);
                ...
                source: new ol.source.XYZ({
                    ...
                    tileUrlFunction: (coordinate) => {
                        ...
                        if (this.regionMap.hasTile(tileX, tileY, worldZoom)) {
                            const url = ('assets/tiles/zoom.{z}/{xd}/{yd}/tile.{x}.{y}.' + this.#options.imageFormat)
                                .replace('{z}', worldZoom)
        ...
        viewport?.addEventListener('wheel', (event) => {
            ...
            const nextZoom = Math.max(0, Math.min(mapZoomLevels, currentZoom + zoomDelta));
